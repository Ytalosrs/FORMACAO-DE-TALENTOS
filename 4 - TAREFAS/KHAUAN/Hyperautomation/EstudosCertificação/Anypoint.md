...
## 1. O que significa “Visibilidade”

Quando você publica uma API no Exchange, você define:

> **Quem pode ver e consumir essa API**

Isso é a **visibilidade**

---

##  2. Private Portal (Privado)

###  O que é

- A API fica visível **somente dentro da organização**
...
###  Papel na arquitetura
 ...
 Base da **Application Network interna**

- Times compartilham APIs
- Reuso acontece dentro da empresa
...
- API de cliente (Customer API)
- API de pedidos (Order API)

 Só sistemas internos usam

---

##  3. Public Portal (Público)

###  O que é

- A API fica visível **externamente**
- Desenvolvedores fora da empresa podem ver
...
Expande a Application Network para fora
...
- API de parceiros (Partner API)
- API pública de serviços

---

##  4. Diferença chave (o que realmente importa)

|Aspecto|Private|Public|
|---|---|---|
|Acesso|Interno|Externo|
|Segurança|Alta (restrita)|Controlada via policies|
|Uso|Reuso interno|Integração externa|
|Exposição|Não visível fora|Visível publicamente|

---

##  5. Relação com API-led Connectivity

Isso conecta direto com:

- **System APIs** → sempre Private
- **Process APIs** → geralmente Private
- **Experience APIs** → podem ser Public

---

##  6. Ponto crítico (muito cobrado)

 Nem toda API deve ser pública

O material de boas práticas reforça:

- governança
- controle de acesso
- segurança via políticas

---

##  Exemplo completo (arquitetura)

### Cenário:

Empresa expõe dados para parceiros

---

### Estrutura:

- System API → banco de dados (**Private**)
- Process API → lógica de negócio (**Private**)
- Experience API → Partner API (**Public**)

 Só a camada necessária é exposta
...
- **Private Portal** → compartilhamento interno controlado
- **Public Portal** → exposição externa de APIs

---

## Insight mais importante

 Visibilidade NÃO é só acesso

 É uma decisão de **arquitetura + segurança + estratégia de reuso**